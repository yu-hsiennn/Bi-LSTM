    ...
    def train(self, model, part, train_, test_, save_path):
        ...
        for epoch in range(self.hyperparams['epochs']):
            ...
            for i, (x, y) in enumerate(tqdm(train_)): 
                ...
                # loss_angle = torch.tensor(0).to(DEVICE)
                loss_ = loss_angle + loss_vel + loss_kl
        ...
        with open(model_path + '/mean.pkl', 'wb')as fpick:
            pickle.dump(mean_list, fpick)  
        with open(model_path + '/log_var.pkl', 'wb')as fpick:
            pickle.dump(log_var_list, fpick)  
        print("done!")
